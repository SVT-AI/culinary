...
const RECIPES = [
  {
    ...
    kcal100: 130,
    portionWeight: 200,
    macros: {"protein": 6, "fat": 10, "carbs": 3},
  ...
  },
  {
    ...
    kcal100: 55,
    portionWeight: 300,
    macros: {"protein": 1.5, "fat": 2, "carbs": 7},
  ...
  },
  {
    ...
    kcal100: 180,
    portionWeight: 300,
    macros: {"protein": 5, "fat": 8, "carbs": 20},
  ...
  },
  {
    ...
    kcal100: 450,
    portionWeight: 80,
    macros: {"protein": 5, "fat": 24, "carbs": 52},
  ...
  },
  {
    ...
    kcal100: 210,
    portionWeight: 180,
    macros: {"protein": 12, "fat": 9, "carbs": 18},
  ...
  },
  {
    ...
    kcal100: 110,
    portionWeight: 250,
    macros: {"protein": 4, "fat": 3, "carbs": 18},
  ...
  },
  {
    ...
    kcal100: 120,
    portionWeight: 400,
    macros: {"protein": 6, "fat": 4, "carbs": 14},
  ...
  },
  {
    ...
    kcal100: 320,
    portionWeight: 120,
    macros: {"protein": 5, "fat": 20, "carbs": 29},
  ...
  },
  {
    ...
    kcal100: 160,
    portionWeight: 250,
    macros: {"protein": 5, "fat": 6, "carbs": 20},
  ...
  },
  {
    ...
    kcal100: 140,
    portionWeight: 180,
    macros: {"protein": 2, "fat": 8, "carbs": 16},
  ...
  },
  {
    ...
    kcal100: 95,
    portionWeight: 250,
    macros: {"protein": 6, "fat": 6, "carbs": 5},
  ...
  },
  {
    ...
    kcal100: 55,
    portionWeight: 300,
    macros: {"protein": 2.5, "fat": 2, "carbs": 6},
...
// --- KCAL MODE & NUTRITION HELPERS ---
function kcalPerPortion(recipe){
  if (!recipe.kcal100 || !recipe.portionWeight) return null;
  return Math.round((recipe.kcal100 * recipe.portionWeight) / 100);
}
function kcalText(recipe){
  const mode = store.kcalMode;
  if (mode === 'portion') {
    const val = kcalPerPortion(recipe);
    if (val) return `${val} kcal/porcja`;
  }
  return `${recipe.kcal100 || '?'} kcal/100g`;
}
function macrosForMode(recipe){
  if (!recipe.macros) return null;
  const { protein, fat, carbs } = recipe.macros;
  if (store.kcalMode === 'portion' && recipe.portionWeight) {
    const ratio = recipe.portionWeight / 100;
    return { protein: +(protein*ratio).toFixed(1), fat: +(fat*ratio).toFixed(1), carbs: +(carbs*ratio).toFixed(1) };
  }
  return { protein, fat, carbs };
}
function energySplit({protein, fat, carbs}){
  const kcalP = protein*4, kcalF = fat*9, kcalC = carbs*4;
  const total = kcalP + kcalF + kcalC || 1;
  return {
    kcalP, kcalF, kcalC, total,
    pctP: Math.round(kcalP/total*100),
    pctF: Math.round(kcalF/total*100),
    pctC: Math.round(kcalC/total*100),
  };
}
...
function render(recipes) {
  ...
  for (const r of recipes) {
    ...
    meta.className = 'meta';
    meta.innerHTML = chip('⏱', r.time + ' min') + chip('⭐', r.difficulty) + (r.kcal100 ? chip('🔥', kcalText(r)) : '');
...
// --- KCAL TOGGLE ---
function applyKcalModeUI(){
  if (!kcalToggle || !kcalModeLabel) return;
  const mode = store.kcalMode;
  kcalToggle.checked = (mode === 'portion');
  kcalModeLabel.textContent = mode === 'portion' ? 'porcja' : '100 g';
}
kcalToggle?.addEventListener('change', () => {
  store.kcalMode = kcalToggle.checked ? 'portion' : '100g';
  applyKcalModeUI();
  applyKcalModeUI();
  applyFilters(); // re-render cards
  // update lightbox/detail if open
  if (currentRecipe) {
    if (!document.getElementById('detail-panel').classList.contains('hidden')) openDetail(currentRecipe, { push: false });
    if (!document.getElementById('lightbox').classList.contains('hidden')) openLightbox(currentRecipe, { push: false });
  }
});
...
function openLightbox(recipe, { push = false } = {}) {
  ...
  lbCap.innerHTML = `<strong>${recipe.title}</strong><br>${recipe.desc}<br>${chip('⏱', recipe.time + ' min')} ${chip('⭐', recipe.difficulty)} ${recipe.kcal100 ? chip('🔥', kcalText(recipe)) : ''}`;
...
const macroUnitLabel = document.getElementById('macro-unit-label');
const macroLegend = document.getElementById('macro-legend');

function drawMacroChart(recipe){
  if (!macroCanvas || !recipe.macros) return;
  const ctx = macroCanvas.getContext('2d');
  // HiDPI scale
  const ratio = window.devicePixelRatio || 1;
  const w = macroCanvas.clientWidth || 360;
  const h = macroCanvas.clientHeight || 180;
  macroCanvas.width = Math.round(w * ratio);
  macroCanvas.height = Math.round(h * ratio);
  ctx.scale(ratio, ratio);
  ctx.clearRect(0,0,w,h);

  const cs = getComputedStyle(document.documentElement);
  const cP = cs.getPropertyValue('--macro-protein').trim() || '#6ab7ff';
  const cF = cs.getPropertyValue('--macro-fat').trim() || '#ff8a65';
  const cC = cs.getPropertyValue('--macro-carbs').trim() || '#ffd54f';

  const m = macrosForMode(recipe);
  if (!m) return;
  const es = energySplit(m);
  macroLegend.textContent = `B: ${m.protein} g • T: ${m.fat} g • W: ${m.carbs} g`;

  const bars = [
    {label:'Białko', val: es.pctP, color: cP},
    {label:'Tłuszcz', val: es.pctF, color: cF},
    {label:'Węglowodany', val: es.pctC, color: cC},
  ];
  const margin = {t: 18, r: 14, b: 24, l: 14};
  ...
  bars.forEach((b, i) => {
    ...
    ctx.fillRect(x0, y, innerW, barH);
    // bar
    ctx.fillStyle = b.color;
    ctx.fillRect(x0, y, innerW * (b.val/100), barH);
    // label
    ctx.fillStyle = getComputedStyle(document.body).color;
    ctx.fillText(`${b.label} — ${b.val}%`, x0 + 6, y + barH - 7);
  });
}
...
function openDetail(recipe, { push=false } = {}) {
  ...
  dMeta.innerHTML = chip('⏱', recipe.time + ' min') + chip('⭐', recipe.difficulty) + (recipe.kcal100 ? chip('🔥', kcalText(recipe)) : '') + (recipe.tags?.length ? chip('Tagi', recipe.tags.map(t=>'#'+t).join(' ')) : '');
  ...
  // Macro chart + unit label
  if (macroUnitLabel) macroUnitLabel.textContent = store.kcalMode === 'portion' ? 'porcję' : '100 g';
  drawMacroChart(recipe);
...
const CONTACT_EMAIL = (() => {
  const rev = 't92YuwWah12ZApWZpNWYtl3azJXYr9Gd';
  try { return atob(rev.split('').reverse().join('')); }
  catch { return 'tokarskymaciej' + '@' + 'gmail.com'; }
})(); // <- podmień na swój adres
...
const cHP = document.getElementById('cf-company');
...
cForm?.addEventListener('submit', (e)=>{
  ...
  if (!ok) return;
  // Honeypot check — jeśli pole wypełnione, przerywamy (bot)
  if (cHP && cHP.value.trim()) { cStatus.textContent = 'Błąd wysyłki.'; return; }
...
// --- BOT SLIDER ---
const slider = document.getElementById('bot-check');
let sDown = false, sStartX = 0, sValue = 0; // 0..100
const sTrack = slider?.querySelector('.slider-track');
const sFill = slider?.querySelector('.slider-fill');
const sHandle = slider?.querySelector('.slider-handle');
const sHint = slider?.querySelector('.slider-hint');

function setSlider(val) {
  sValue = Math.max(0, Math.min(100, val));
  if (sFill) sFill.style.width = sValue + '%';
  if (sHandle && sTrack) {
    const maxX = sTrack.clientWidth - sHandle.clientWidth - 6; // paddings
    sHandle.style.left = (3 + (maxX * sValue / 100)) + 'px';
  }
  if (slider) slider.setAttribute('aria-valuenow', String(Math.round(sValue)));
}
...
function finishSlider() {
  ...
  if (sHint) sHint.textContent = 'Potwierdzono ✔';
  slider.dataset.verified = '1';
}

function onMove(clientX) {
  if (!sDown || !sTrack || !sHandle) return;
  const rect = sTrack.getBoundingClientRect();
  const x = Math.max(rect.left, Math.min(clientX, rect.right)) - rect.left;
  const pct = (x - sHandle.clientWidth/2) / (rect.width - sHandle.clientWidth) * 100;
  setSlider(pct);
  if (sValue > 96) { sDown = false; finishSlider(); }
}

sHandle?.addEventListener('pointerdown', (e) => {
  sDown = true; slider?.classList.add('dragging'); sStartX = e.clientX; sHandle.setPointerCapture(e.pointerId);
});
window.addEventListener('pointermove', (e) => { if (sDown) onMove(e.clientX); });
window.addEventListener('pointerup', (e) => { if (!sDown) return; sDown = false; slider?.classList.remove('dragging'); if (sValue > 96) finishSlider(); });

// keyboard support
slider?.addEventListener('keydown', (e) => {
  if (slider.getAttribute('aria-disabled') === 'true') return;
  const step = (e.key === 'ArrowLeft' || e.key === 'ArrowRight') ? 10 : 0;
  if (e.key === 'ArrowRight') { setSlider(sValue + step); e.preventDefault(); }
  if (e.key === 'ArrowLeft') { setSlider(sValue - step); e.preventDefault(); }
  if (e.key === 'Home') { setSlider(0); e.preventDefault(); }
  if (e.key === 'End') { setSlider(100); e.preventDefault(); finishSlider(); }
  if (e.key === ' ' || e.key === 'Enter') { setSlider(100); finishSlider(); e.preventDefault(); }
});
