...
// === CALORIE CALC =============================
const CAL_DB = {
  "pomidor": 18, "mozzarella": 280, "bazylia": 23, "oliwa": 884,
  "dynia": 26, "cebula": 40, "imbir": 80, "bulion": 5, "śmietanka 30%": 300,
  "makaron": 350, "pesto": 470, "parmezan": 431, "kurczak (pierś)": 165,
  "tortilla pszenna": 320, "awokado": 160, "ryż": 360,
  "płatki owsiane": 379, "mleko 2%": 50, "mleko 3.2%": 62, "napój migdałowy": 17,
  "banan": 89, "jagody": 57, "jajko": 143, "truskawki": 33,
  "cukier": 400, "masło": 717, "czekolada gorzka": 546, "mąka pszenna": 364,
  "ser żółty": 350, "ziemniaki": 77, "brokuły": 34, "czosnek": 149,
  "ser mascarpone": 435, "kakao": 228, "śmietanka": 300, "wanilia": 288
};

// selectors
const calName = document.getElementById('cal-name');
const calGrams = document.getElementById('cal-grams');
const calKcal100 = document.getElementById('cal-kcal100');
...
function saveCalc() {
  ...
  const rows = [...calBody.querySelectorAll('tr')].map(tr => {
    ...
    const g = parseFloat(tr.querySelector('.c-grams').value) || 0;
    const k100 = parseFloat(tr.querySelector('.c-k100').value) || 0;
    return { name, g, k100 };
  });
  localStorage.setItem('CALC_ROWS', JSON.stringify(rows));
}
...
function addRow({ name='', grams='', k100=null } = {}) {
  const tr = document.createElement('tr');
  tr.innerHTML = `
    <td><input class="c-name" list="cal-db" placeholder="Składnik" value="${name}"></td>
    <td><input class="c-grams" type="number" min="0" step="1" placeholder="0" value="${grams}"></td>
    <td><input class="c-k100" type="number" min="0" step="1" placeholder="0" value="${k100!==null?k100:''}"></td>
    <td class="c-kcal">0</td>
    <td><button class="row-remove" aria-label="Usuń wiersz">✕</button></td>
  `;
  calBody.appendChild(tr);

  const nameEl = tr.querySelector('.c-name');
  const gramsEl = tr.querySelector('.c-grams');
  const k100El = tr.querySelector('.c-k100');

  // auto-fill kcal/100g if known
  nameEl.addEventListener('change', () => {
    const key = nameEl.value.trim().toLowerCase();
    if (k100El.value === '' && CAL_DB[key] != null) {
      k100El.value = CAL_DB[key];
    }
    recalc();
  });
  gramsEl.addEventListener('input', recalc);
  k100El.addEventListener('input', recalc);
  tr.querySelector('.row-remove').addEventListener('click', () => { tr.remove(); recalc(); });

  // initial autofill
  if (k100 === null) {
    const key = (name||'').trim().toLowerCase();
    if (CAL_DB[key] != null) k100El.value = CAL_DB[key];
  }
  recalc();
}
...
calAdd?.addEventListener('click', (e) => {
  ...
  const grams = calGrams.value;
  const k100 = calKcal100.value ? parseFloat(calKcal100.value) : null;
  if (!name) { calName.focus(); return; }
  addRow({ name, grams, k100 });
  calName.value=''; calGrams.value=''; calKcal100.value='';
  calName.focus();
});
...
(function restoreCalc(){
  try {
    const saved = JSON.parse(localStorage.getItem('CALC_ROWS')||'[]');
...
detailToCal?.addEventListener('click', () => {
  if (!currentRecipe) return;
  (currentRecipe.ingredients||[]).forEach(name => addRow({ name, grams:'', k100:null }));
  // Scroll to tool
